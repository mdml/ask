...
def archive(data):
    with tarfile.open(fileobj=io.BytesIO(data)) as tar:
        assert all(m.isfile() or m.isdir() or m.issym() for m in tar.getmembers())
...
def main():
    ...
    for short, name, version, sub in [
        ...
        assert digest(data) == selected["cksum"] and not selected["yanked"]
        ...
        assert all(not p.get("source") or p["source"] == "registry+https://github.com/rust-lang/crates.io-index"

...
The provisional composition rule joins the instruction, a blank line, and the payload as `"{instruction}\n\n{payload}"`. Prompt words are joined with single spaces and surrounding whitespace is trimmed in every mode; the UTF-8 payload is preserved byte for byte, including leading whitespace and trailing newlines. An empty or whitespace-only payload, such as stdin redirected from `/dev/null`, leaves the instruction alone. Prompt words that are empty or whitespace-only are a usage error whether stdin is a terminal or redirected, and nothing is read. This is a compositional convenience, not a security boundary.

The provisional terminal submission rule prints `ask> ` on stderr before the first line and reads until EOF (Ctrl-D on an empty line submits). Ctrl-C retains its default behavior; `ask` installs no SIGINT handler. Collected text is submitted unchanged. The provisional empty-input rule rejects empty or whitespace-only input from either a terminal submission or redirected stdin without prompt words, with exit 2 and a one-line stderr diagnostic; no provider request is sent.

Stdin must be valid UTF-8 and is read to EOF without an artificial size cap. Invalid UTF-8 and input read failures exit 1 with one `ask: ...` stderr diagnostic line, empty stdout, and no provider request; invalid bytes are never converted lossily.

...
- `main`: existing ruleset recorded by the delivery draft; unchanged here and not copied here. Blocks deletion and force-pushes, requires merge-commit-only pull requests, `verify-full` and the four target checks, and disables the up-to-date ancestry requirement. Recheck the live settings before integration.
- `staging`: existing ruleset retained during the [transition](../../../CONTRIBUTING.md#transition-from-staging); no new work lands there.

...
- `stable.json`: applied and read back on 2026-09-15 as ruleset `23480413`. Blocks deletion of `stable` without bypass actors. The branch contains the owner-authorized `v0.1.0` release. Owner authorization, the full gate, and the security checks in [SECURITY.md](../../../SECURITY.md) remain separate prerequisites for each stable push; this ruleset does not implement them.

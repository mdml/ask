...
| P4 | Stable `v0.1.0` and its Homebrew formula are published; see [stable releases](../guides/stable-releases.md). | Owner Homebrew installation verification. |
| P9 | Terminal presentation and onboarding, task-oriented documentation, the deterministic README demo, the `/ask-doctor` skill, `demo_proof`, and repeatable offline installed-workflow acceptance are implemented and published in stable `v0.1.0`. | Owner Homebrew installation verification. |
...
| P4 | Stable release delivery (see below). | Owner Homebrew installation verification. |
...
|:--|:--|:--|
| Install | Nightly packaging tests; offline packaged `configure check`; stable `v0.1.0` and its Homebrew formula | Owner Homebrew installation verification |
...
| Presentation | `demo_proof` smoke-tests the deterministic terminal recorder; `offline_acceptance` and `just acceptance <binary>` exercise initialization, configuration, queries, replies, switching, inspection, composition, credential isolation, and terminal restoration through the real binary; stable `v0.1.0` contains P9 | Owner Homebrew installation verification |

Record personally run results in PRs; this table lists proof targets, not results.

## Stable and Homebrew delivery (P4)

Stable `v0.1.0` was published from accepted nightly source `3c8ce0e`, followed by its Homebrew formula. The implementation reuses pinned actions and packaging code. The stable guide records the reusable operator steps and security checks required before each publication:

1. Increment the semantic version in `Cargo.toml`, then publish, review, and test a nightly containing that version and the intended changes. Prepare the stable candidate from that nightly plus any reviewed and verified cherry-picked fixes. A push to `stable` reuses the nightly verify, native build/test, package, attest, and draft-verify-publish jobs, with a non-prerelease tag derived from the Cargo version; it refuses an existing tag.
...
4. Homebrew: update the public `mdml/homebrew-tap` repository as part of each authorized stable release, without adding a cross-repository automation credential. The formula selects the four stable archives by URL and SHA-256 from `SHA256SUMS`, updated after publication; no new build path.
...
- Stable `v0.1.0` and its formula are published after authorization and candidate security checks; owner Homebrew installation verification remains pending.

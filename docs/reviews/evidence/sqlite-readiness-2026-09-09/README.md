...
This directory records the 2026-09-09 follow-up, separate from the September 6 evidence. The [assessment](../../sqlite-readiness-2026-09-09.md) is the interpretation and owner handoff; these files are dated observations, not adoption approval.
...
| `linux-x86_64.json` | Local isolated release probe output, native options, linkage and hashes. Three other targets are pending. |
...
The dedicated workflow emits the same sanitized JSON in its job log and GitHub step summary. It does not upload raw artifacts or add action pins. The manager should retain all four successful results with the tested commit, public GitHub run URL, and runner labels after running the workflow. Until then, do not replace the three pending entries with passes.

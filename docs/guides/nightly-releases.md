...
The [nightly release workflow](../../.github/workflows/nightly-release.yml) builds installable prereleases from protected `main` at 06:37 UTC daily or through **Run workflow** with `main` selected. Other refs and repositories are skipped. A scheduled or ordinary manual run first compares the `main` commit with the source of the most recent successfully published nightly and skips tagging, verification, building, attestation, and publication when they match; see [Unchanged source](#unchanged-source). Dependency reporting remains in the separate `nightly` workflow, including the [report-only native SQLite monitor](../development/sqlite-monitoring.md).

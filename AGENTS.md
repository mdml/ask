...
- Promotion to `stable` requires the owner's authorization for each release; the managing agent performs it once authorized. The `stable` branch and live stable publication do not exist until that first authorized push; the workflow is prepared on the beta umbrella and `docs/development/rulesets/stable.json` records the applied deletion protection. `SECURITY.md` and `docs/guides/stable-releases.md` list the checks and operator steps.

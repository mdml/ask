...
- `main` is the nightly channel. It receives only fully gated promotion PRs that carry exactly `staging`'s content, merged by merge commit after a named proof or milestone passes. A promotion PR normally comes from `staging` itself; when `main` has changes that conflict with `staging`, it comes from a `promote/<name>` branch created from `staging` with `main` merged in and resolved so that its tree is identical to `staging`'s. `main` rejects direct pushes, force-pushes, and deletion, and requires `verify-full` plus the four supported-target checks. Its up-to-date ancestry requirement is deliberately disabled: promotion merge commits belong on `main`, while `staging` keeps linear history. Do not rebase or merge `main` into protected `staging` to update a promotion. Verify the combined promotion revision against the current `main`; if either branch moves, refresh the combined revision and its checks before owner merge. Stable releases are tags from `main`.

...
`src/store.rs` owns the SQLite database, and `src/store_recall.rs` adds its read-only views and current-thread selection. `ask new` and `ask reply` open or create it; `ask thread`, `ask switch`, and `ask stats` open it only when the file exists, using SQLite's open without its create flag so a file removed concurrently is never recreated. Each process uses one connection with synchronous calls outside the streaming section. Schema version 2 is created in one transaction that sets `PRAGMA user_version = 2`; a version 1 database is upgraded in one transaction that adds the `history_expiry` table. An unversioned file is accepted only when it is empty, and a newer or unrecognized version is refused before anything is written. Foreign keys are on.

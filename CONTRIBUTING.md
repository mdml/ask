...
Promotion to `stable` requires the owner's authorization for each release; the managing agent performs the push once authorized. `v0.1.0` is the first stable release. The applied deletion-block ruleset is in `docs/development/rulesets/stable.json`, and `SECURITY.md` lists the checks a push to `stable` must pass; see `docs/guides/stable-releases.md`.
...
cargo +1.97.1 test --locked --test provider_proof
cargo +1.97.1 test --locked --test doctor_proof
cargo +1.97.1 test --locked --test live_provider_check
cargo +1.97.1 test --locked --test demo_proof
cargo +1.97.1 test --locked --test offline_acceptance
just acceptance target/debug/ask
```

The configure proof drives `ask init` with redirected stdin, validates and applies candidate files and stdin, and queries a fake provider using the installed configuration. Sentinel cases verify that validation diagnostics never echo candidate-controlled text. Python standard-library helpers in `tests/support/` use a PTY to prove terminal usage errors and that the `init` provider menu handles arrow-key selection, Escape, and Ctrl-C while restoring terminal state; FIFO rendezvous proves destination changes during candidate reading and concurrent writer exclusion; and the flushed initialization confirmation prompt proves interaction with `apply`. They also exercise invalid UTF-8 from files and stdin. On Linux, a `ptrace` helper pauses the real binary at the final link/rename syscall and temporarily denies directory writes, proving creation and replacement failures preserve the destination and clean up temporary files. The same syscall boundary proves that creation never clobbers a destination appearing after the snapshot check. This requires permission to trace a child process; it is not a production test hook. File-size limits separately exercise temporary-write failures. The query proof exercises response, streaming, error, and timeout behavior, plus stdin-only queries and exact instruction/payload composition at the fake-provider boundary. It checks UTF-8 byte preservation, empty and unreadable input, invalid UTF-8, stdout/stderr separation, the final newline, provider and streaming failures with piped input, and quiet early pipe closure. A Python PTY helper proves that terminal prompt words never wait for input and that the multiline prompt uses stderr, accepts multiple lines through EOF, checks partial-line double Ctrl-D submission (the process remains alive and silent for 0.5 seconds after the first Ctrl-D; after line-kill and a second Ctrl-D, exactly one request contains the original partial line), rejects empty submissions without contacting the provider, and, with SIGINT delivered directly as the terminal driver does for Ctrl-C, terminates by that signal with empty stdout and no provider request. Both proofs use deterministic fixtures and a programmable HTTP provider on loopback; they need permission to bind local sockets but require no external network access, provider credentials, or paid services. Shared helpers live in `tests/support/`.

The continue proof runs `ask new` and `ask reply` as separate processes of the real binary against a fake provider that answers successive requests with successive scenarios and records every request. It asserts the exact messages, order, system prompt, and model each reply sends; that replies keep the thread's profile snapshot after the configuration changes, becomes invalid, or is removed, warning that history expiry was skipped in the last two cases; the command aliases; that a reply held at the provider while `ask new` creates another thread still sends and appends to the thread it started with and becomes current when it finishes last; piped reply input; the missing-current-thread error without a request; that a failure before answer text leaves the current thread unchanged; that partial turns are recorded but not replayed and empty answers are complete turns; quiet early pipe closure with a recorded partial turn; and the database location, permissions, and absence of the credential value. A gated fake-provider response holds the answer after the request arrives while the proof makes the data directory and database read-only, proving that a delivered answer that cannot be recorded exits 1 with stdout preserved, a one-line diagnostic, and the database unchanged. That case skips when the current user can write despite the permissions, for example as root.

The recall proof runs `ask thread`, `ask switch`, `ask stats`, and queries as separate processes of the real binary against the fake provider. It asserts the exact thread rendering, including an incomplete-turn marker; the missing-current-thread error without creating a database; quiet early closure of the reader; selection by id and by a redirected menu answer, with the next reply sending the selected thread's turns; rejected ids and selections leaving the current thread unchanged; and exact `ask stats` output. Its PTY menu cases cover arrow-key selection, Escape, Ctrl-C, terminal restoration, the `TERM=dumb` fallback, bounded and wrapped viewports, the minimum-height diagnostic, and rendering the selected snapshot when the stored current thread changes during selection. Time-dependent cases rewrite recorded timestamps in the database, either to fixed instants for rendered times or to whole days before the present for expiry, so results do not depend on when the proof runs. The expiry cases prove that history is kept indefinitely by default; that enabled expiry defaults to 90 days, uses each thread's newest turn, removes whole threads only in `ask new` and `ask reply`, and keeps statistics and provider health with a cleared-thread count; that a reply to an expired current thread reports no current thread without a request; that a reply with valid configuration applies expiry to other threads; that replies with missing or invalid configuration warn that expiry was skipped, remove nothing, and keep using the captured profile; and that a reply whose thread another query expires while the answer streams exits 1 with stdout preserved and records none of its turn, statistics, or current-thread change. A Python PTY helper, `tests/support/recall_process.py`, proves at the terminal prompt that Ctrl-C and blank submissions to `ask new` and `ask reply` leave history unchanged (redirected blank input is checked too), that a captured thread expired by the submitted reply fails without a request, and that a reply continues the thread current when it started although another process runs `ask switch` while it waits for input.

The provider proof runs the real binary once per supported `kind` against hand-written server-sent-event fixtures in each provider's wire format, derived from provider documentation and Rig's own tests rather than captured from live services. For each kind it asserts the request path, where the credential travels (and that it travels nowhere else), the verbatim model identifier, how the system prompt and reply history are encoded, streamed text and usage, the default and explicit output-token limit fields, output-limit truncation as a partial turn excluded from the next reply, credential-free 401 and 429 diagnostics, and refused redirects that never reach the destination. It also proves Gemini model and credential percent-encoding in the URL, redaction of a percent-encoded credential from a transport diagnostic, OpenRouter's in-band stream error as a partial turn, and a reply that continues a thread recorded by a version 1 database. Live compatibility against the real services is separate and not part of these proofs.

The doctor proof runs the real binary for offline diagnostics and opt-in live checks against the fake provider. Offline cases assert valid and invalid configuration, missing credentials, side-effect-free path and storage inspection including damaged databases, and the `d` alias. Live cases assert `--live` and `--live --all` against the fake provider with the doctor live prompt and output-token cap; they need loopback sockets but no external network, provider credentials, or paid services.
...
The demo proof smoke-tests the deterministic terminal recorder against the built binary. The offline acceptance proof runs the installed workflow through the real binary, including interactive initialization and switching, configuration replacement, captured-profile replies, thread inspection, shell and pipe composition, credential isolation, and terminal-state restoration. Run the same walkthrough against any already-built candidate with `just acceptance <binary>`. See the [terminal demo guide](docs/development/demo.md) for recording and rendering commands and their evidence boundaries.

## Local storage

`src/store.rs` owns the SQLite database, and `src/store_recall.rs` adds its read-only views and current-thread selection. `ask new` and `ask reply` open or create it; `ask thread`, `ask switch`, and `ask stats` open it only when the file exists, using SQLite's open without its create flag so a file removed concurrently is never recreated. Each process uses one connection with synchronous calls outside the streaming section. Schema version 4 is created in one transaction that sets `PRAGMA user_version = 4`. A version 1 database is migrated in one immediate transaction through version 2, which adds the `history_expiry` table, to version 3, which adds the nullable `threads.max_output_tokens` snapshot column, to version 4, which adds nullable `provider_health.last_success_source` and `last_failure_source` columns; existing threads keep no output limit and existing health rows keep NULL sources. A version 2 database with history expiry but no output-token column is upgraded through version 3 to version 4 the same way. A version 3 database gains the health-source columns on upgrade. An unpublished prototype that marked version 2 with only an output-token column is refused. An unversioned file is accepted only when it is empty, and a newer or unrecognized version is refused before anything is written. Offline `doctor` never migrates storage. Foreign keys are on.

The database uses the rollback journal (`journal_mode = DELETE`), `synchronous = FULL`, and a 1000 ms busy timeout. Each query performs one short write transaction after its answer, preceded by one short expiry transaction when expiry is enabled, so write-ahead logging's concurrent-reader benefit does not matter for `ask`. The rollback journal leaves no persistent `-wal` or `-shm` files next to the database. Offline `doctor` checks both journal-format header bytes and companion files before opening SQLite read-only, and reports a limited check when safe deeper inspection is unavailable; see the [SQLite adoption record](docs/reviews/sqlite-adoption-2026-09-10.md#offline-diagnostics-and-the-read-only-wal-path). Full synchronization makes each recorded turn durable once the command exits.

Storage unit tests in `src/store_tests.rs` cover paths and permissions, schema creation, version 1 and recall version 2 upgrades to version 4, version 3 upgrades to version 4 with nullable health-source columns, refusal of unpublished prototype version 2 layouts and of newer versions, opening only existing databases without creating a file, offline read-only inspection without migration or side effects, reopening, expiry by newest turn at the exact boundary, thread ids that are never reused after expiry, the thread view, recent threads and selection, the statistics summary, turn order, snapshot round trips including explicit output limits, rollback of every table on an injected failure, provider-health identity and source recording, and text-free statistics. An ignored test measures open cost for first creation and warm reopen; run it locally with `cargo test --release --lib store::tests::startup -- --ignored --nocapture`.

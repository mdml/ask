...
`src/store.rs` owns the SQLite database, and `src/store_recall.rs` adds its read-only views and current-thread selection. `ask new` and `ask reply` open or create it; `ask thread`, `ask switch`, and `ask stats` open it only when the file exists, using SQLite's open without its create flag so a file removed concurrently is never recreated. Each process uses one connection with synchronous calls outside the streaming section. Schema version 3 is created in one transaction that sets `PRAGMA user_version = 3`. A version 1 database is migrated in one immediate transaction through version 2, which adds the `history_expiry` table, to version 3, which adds the nullable `threads.max_output_tokens` snapshot column; existing threads keep no limit. A version 2 database with history expiry but no output-token column is upgraded to version 3 the same way. An unpublished prototype that marked version 2 with only an output-token column is refused. An unversioned file is accepted only when it is empty, and a newer or unrecognized version is refused before anything is written. Foreign keys are on.

The database uses the rollback journal (`journal_mode = DELETE`), `synchronous = FULL`, and a 1000 ms busy timeout. Each query performs one short write transaction after its answer, preceded by one short expiry transaction when expiry is enabled, so write-ahead logging's concurrent-reader benefit does not matter for `ask`. The rollback journal leaves no persistent `-wal` or `-shm` files next to the database, and with no `-wal` file present SQLite never opens a WAL connection, which keeps a future side-effect-free offline diagnostic away from the read-only WAL recovery path described in the [SQLite adoption record](docs/reviews/sqlite-adoption-2026-09-10.md#offline-diagnostics-and-the-read-only-wal-path). Full synchronization makes each recorded turn durable once the command exits.

Storage unit tests in `src/store_tests.rs` cover paths and permissions, schema creation, version 1 and recall version 2 upgrades to version 3, refusal of unpublished prototype version 2 layouts and of newer versions, opening only existing databases without creating a file, reopening, expiry by newest turn at the exact boundary, thread ids that are never reused after expiry, the thread view, recent threads and selection, the statistics summary, turn order, snapshot round trips including explicit output limits, rollback of every table on an injected failure, provider-health identity, and text-free statistics. An ignored test measures open cost for first creation and warm reopen; run it locally with `cargo test --release --lib store::tests::startup -- --ignored --nocapture`.
